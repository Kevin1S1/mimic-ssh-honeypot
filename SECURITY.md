...
- A global cap on total real-disk quarantine usage (`quarantine_max_total_bytes`, default `max_upload_bytes * 64`) prevents multi-session or parallel upload floods from filling the host disk.
...
- **Per-command output cap** (`MAX_COMMAND_OUTPUT_BYTES = 1 MiB`) truncates any single command's output at the dispatch layer, preventing memory amplification from `cat`/`find`/`grep` on large VFS content. The cap is shared between the command's stdout and stderr, so splitting what it writes across the two streams cannot buy a second helping. The same ceiling applies to a whole command line, so chaining (`cat big; cat big; …`) cannot multiply it by the number of segments the 4096-byte input limit allows; to the command-substitution output one line may splice into itself (`echo $(cat big)$(cat big)…`); and to the stderr those substitutions accumulate on the way, which is held for the outer line rather than spliced into the value.
- **Command-substitution nesting cap.** `$(…)` and `$((…))` recurse during expansion, before any command is dispatched, so the shell checks the command-nesting cap (`MAX_NESTED_COMMANDS = 16`) itself rather than relying on `dispatch` to notice. A deeply nested line (`$($($(…)))`) would otherwise overflow the stack and abort the whole daemon rather than one session.
...
- **Login grace time** — a connection that has not logged in within 120 s is disconnected (Debian sshd's `LoginGraceTime`), so clients that never authenticate cannot camp on the connection slots for the full session lifetime and blind the sensor.
...
In Docker deployments, this is handled by a lightweight sidecar container. For systemd, a timer + one-shot service unit pair is provided. See [Daily reset](docs/deployment.md#daily-reset) for configuration.
...
| T5 | Disk exhaustion | SCP / SFTP upload flood | Size-capped, SHA-256 content-addressed (dedup), filename sanitised (separators → `_`, then `..` → `_`, control bytes dropped), written `0600` non-exec, per-session quarantine disk-write cap (`max_upload_bytes` × 32), global quarantine disk-usage cap (`quarantine_max_total_bytes`) | Bounded per session and globally; purged by daily reset |
...
| T8 | Hung / zombie sessions | Slowloris, idle hold, unauthenticated slot camping | Idle timeout + 120 s login grace time + absolute `max_session_secs` cap | Bounded |
...
| T10 | Honeypot fingerprinting | Banner/KEX/timing/`/proc`/missing-feature probes | Debian 12 OpenSSH-shaped handshake, persistent host key, readline emulation, dynamic `/proc/uptime` and `/proc/loadavg` generation, response jitter, recon-command coverage, randomised daily restart (not at fixed time), explicit `SSH_MSG_CHANNEL_SUCCESS`/`FAILURE` replies to `pty-req`/`env`/`window-change`/subsystem/agent requests (matching real sshd instead of leaving them unanswered) | Ongoing — anti-detection is an evolving discipline; TCP/IP-stack fingerprinting (TTL, window sizes) is host-kernel territory, addressed by host `sysctl`/firewall tuning, not the application |
...
| T13 | Whole-process hang or abort from an emulated command | Cyclic VFS move (`mv a a/b`), pathological `find -name` glob, deep directory chain walked by `find`/`du`/`tar` | `Vfs::rename` keeps the arena acyclic and `path_of` is arena-bounded; `glob_match` is an iterative single-backtrack matcher instead of exponential recursion; tree depth capped at 128 levels in `Vfs::insert` and `Vfs::rename`, so recursive walkers cannot overflow the stack | Bounded — no unbounded walk or match remains on an attacker-reachable path |
...
| Login grace time | 120 s | `network/ssh.rs` watchdog | Unauthenticated slot exhaustion |
...
| VFS tree depth | 128 | `vfs/mod.rs` | Stack overflow in recursive walkers (aborts the process) |
